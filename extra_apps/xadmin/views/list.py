...
class ListAdminView(ModelAdminView):
    ...
    def get_list_queryset(self):
        ...
        if self.list_prefetch_related: queryset = queryset.prefetch_related(*self.list_prefetch_related)
        # Return the queryset.
        return queryset
    ...
    def result_headers(self):
        ...
        row.cells = [self.result_header(
            field_name, row) for field_name in self.list_display if field_name not in self.list_exclude ]
        return row
        ...
        try:
            ...
            if f is None:
                ...
                if boolean:
                    ...
                    if isinstance(value,str) and hasattr(attr,'value_format'):
                        item.text = mark_safe(utils.format_value_has_link(value, self.list_item_max_length))
                    else:
                        item.text = smart_text(value)
            else:
                if isinstance(f.remote_field, (models.ManyToOneRel,models.ManyToManyField)):
                    field_val = getattr(obj, f.name)
                    if field_val is None:
                        item.text = mark_safe("<span class='text-muted'>%s</span>" % EMPTY_CHANGELIST_VALUE)
                    else:
                        if isinstance(value,Basemodel):
                            item.text=value.get_model_link('list',self.list_item_max_length)
                        else:
                            item.text = field_val
                else:
                    if isinstance(value,str) and value not in self.list_max_exclude_field:
                        item.text=mark_safe(utils.format_value_has_link(value,self.list_item_max_length))
                    else:
    ...
    def result_row(self, obj):
        ...
        row.cells = [self.result_item(
            obj, field_name, row) for field_name in self.list_display if field_name not in self.list_exclude]
        return row
