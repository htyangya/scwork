...
class DetailAdminView(ModelAdminView):
    ...
    def get_model_params(self):
        return "?app_label={0}&model_name={1}&id={2}".format(self.app_label,self.model_name,self.obj.id)

    @filter_hook
    def get_submit_url(self,type):
        return reverse('submit:flow')+self.get_model_params()+f"&type={type}"

    def get_notifies(self):
        notifies=getattr(self.obj,'notifies',None)
        if not notifies:return None
        return notifies.all().order_by('type','-timestamp').select_related('nfrom','recipient')
