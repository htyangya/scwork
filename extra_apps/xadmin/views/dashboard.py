...
import copy
from custom.models import Notify
from common.middlewares import get_onlie_users
...
class Dashboard(CommAdminView):
    ...
    def get_widgets(self):
        ...
        if self.widget_customiz:
            portal_pos = UserSettings.objects.filter(
                user_id=1, key=self.get_portal_key()).order_by('-id')
            if len(portal_pos):
                portal_pos = portal_pos[0].value
                widgets = []

                if portal_pos:
                    user_widgets = dict([(uw.id, uw) for uw in UserWidget.objects.filter(user_id=1, page_id=self.get_page_id()).order_by('-id')])
    ...
    def get_notify(self):
        notifys=Notify.objects.filter(type=0,title__contains="通知")
        return  notifys
    ...
    def get_context(self):
        new_context = {
            ...
            'online_users': get_onlie_users(),
            'notifys':self.get_notify()
        }
